...
// 
//
// Related Topics 树 深度优先搜索 动态规划 二叉树 👍 2106 👎 0
...
public class LC124BinaryTreeMaximumPathSum {
    ...
    class Solution {
        ...
        int oneSidePath(TreeNode root) {
            ...
            int left = oneSidePath(root.left);
            int right = oneSidePath(root.right);
            int maxPathSum = Math.max(root.val, Math.max(root.val + left, Math.max(root.val + right, root.val + left + right)));
